...
function seesaw:init(x, y, r)
	...
	if #self.r > 0 and self.r[1] ~= "link" then
		self.range = seesawdistancefunction(tonumber(self.r[1]))
		table.remove(self.r, 1)
	end
	--DIST1
	if #self.r > 0 and self.r[1] ~= "link" then
		self.dist1 = seesawheightfunction(tonumber(self.r[1]))
		table.remove(self.r, 1)
	end
	--DIST2
	if #self.r > 0 and self.r[1] ~= "link" then
		self.dist2 = seesawheightfunction(tonumber(self.r[1]))
		table.remove(self.r, 1)
	end
	--WIDTH
	if #self.r > 0 and self.r[1] ~= "link" then
		self.width = platformwidthfunction(tonumber(self.r[1]))
		table.remove(self.r, 1)
	end
...
function seesaw:update(dt)
	if self.falloff then
		...
		if self.leftplatform.speedy == 0 then
			if self.leftplatform.y > self.y+self.dist1-0.8 then
				self.leftplatform.speedy = -1
				self.rightplatform.speedy = 1
			elseif self.leftplatform.y < self.y+self.dist1-1.2 then
				self.leftplatform.speedy = 1
				self.rightplatform.speedy = -1
			end
		end
...
function seesaw:fallingoff(side)
	...
	if true then return end --ARCADE
...
function seesaw:draw()
	...
	if self.falloff == false and self.leftplatform.y-self.y >= 0 then
		love.graphics.setScissor((self.x-1-xscroll)*16*scale, (self.y-0.5)*16*scale, 16*scale, math.floor((self.leftplatform.y-self.y-yscroll)*16*scale))
	...
	if self.falloff == false and self.rightplatform.y-self.y >= 0 then
		love.graphics.setScissor((self.x-1+self.range-xscroll)*16*scale, (self.y-0.5)*16*scale, 16*scale, math.floor((self.rightplatform.y-self.y-yscroll)*16*scale))
