function game_load(suspended)
	...
	outputs = { "button", "laserdetector", "pushbutton", "walltimer", "squarewave", "notgate", "orgate", "andgate", "regiontrigger", "actionblock"}
	outputsi = {40, 56, 57, 58, 59, 68, 69, 74, 82, 84, 19, 86, 87, 39}
...
function game_update(dt)
	...
	for i, v in pairs(objects) do
		if i ~= "tile" and i ~= "portalwall" and i ~= "screenboundary" then
			...
			for j, w in pairs(v) do
				...
				elseif (not client and not server) and w.autodelete then
	...
	if not levelfinished and firestarted and (not objects["bowser"][1] or (objects["bowser"][1].backwards == false and objects["bowser"][1].shot == false and objects["bowser"][1].fall == false)) then
		...
		while firetimer > firedelay do
			...
			table.insert(objects["fire"], fire:new(xscroll + width, math.random(3)+7))
		end
	end
	...
	if not levelfinished and bulletbillstarted then
		...
		while bulletbilltimer > bulletbilldelay do
			...
			table.insert(objects["bulletbill"], bulletbill:new(xscroll+width+2, math.random(4, 12), "left"))
		end
	end
...
function game_draw()
	for split = 1, #splitscreen do
		...
		if mapheight < height+1 then
			ytodraw = math.ceil(mapheight/#splitscreen)
		else
		...
		for y = 1, ytodraw do
			for x = 1, xtodraw do
				...
				elseif tilequads[tilenumber].coin then --coin
					love.graphics.drawq(coinimg, coinquads[spriteset][coinframe], math.floor((x-1-math.mod(xscroll, 1))*16*scale), ((y-1-math.mod(yscroll, 1)-bounceyoffset)*16-8)*scale, 0, scale, scale)
				...
				if editormode then
					...
					if entitylist[map[x][y][2]] == "platform" then
						...
						if rightclickm and rightclickm.tx == x and rightclickm.ty == y then
							...
							dist = platformdistancefunction(tonumber(rightclickm.t[6].value))
						else
							dir = map[x][y][3]
							dist = platformdistancefunction(tonumber(map[x][y][5]))
						end
		...
		if levelfinished and levelfinishtype == "castle" then
			if levelfinishedmisc2 == 1 then
				if levelfinishedmisc >= 1 then
					properprint("thank you mario!", math.floor(((mapwidth-12-xscroll)*16-1)*scale), (4.5-yscroll)*16*scale)
				end
				if levelfinishedmisc == 2 then
					properprint("but our princess is in", math.floor(((mapwidth-13.5-xscroll)*16-1)*scale), (6.5-yscroll)*16*scale) --say what
					properprint("another castle!", math.floor(((mapwidth-13.5-xscroll)*16-1)*scale), (7.5-yscroll)*16*scale) --bummer.
				end
			else
				if levelfinishedmisc >= 1 then
					properprint("thank you mario!", math.floor(((mapwidth-12-xscroll)*16-1)*scale), (4.5-yscroll)*16*scale)
				...
				if levelfinishedmisc >= 2 then
					properprint("your quest is over.", math.floor(((mapwidth-12.5-xscroll)*16-1)*scale), (6-yscroll)*16*scale)
				end
				if arcade then
					if levelfinishedmisc >= 3 then
						properprint("we hope you enjoyed", math.floor(((mapwidth-13.5-xscroll)*16-1)*scale), (7-yscroll)*16*scale)
					end
					if levelfinishedmisc >= 4 then
						properprint("the gamescom demo", math.floor(((mapwidth-12-xscroll)*16-1)*scale), (8.5-yscroll)*16*scale)
					end
					if levelfinishedmisc == 5 then
						properprint("of mari0! -stabyourself.net", math.floor(((mapwidth-15-xscroll)*16-1)*scale), (9.5-yscroll)*16*scale)
					end
				else
					if levelfinishedmisc >= 3 then
						properprint("we present you a new quest.", math.floor(((mapwidth-14.5-xscroll)*16-1)*scale), (7-yscroll)*16*scale)
					end
					if levelfinishedmisc >= 4 then
						properprint("push button b", math.floor(((mapwidth-11-xscroll)*16-1)*scale), (8.5-yscroll)*16*scale)
					end
					if levelfinishedmisc == 5 then
						properprint("to play as steve", math.floor(((mapwidth-12-xscroll)*16-1)*scale), (9.5-yscroll)*16*scale)
					end
				end
			end
						...
						if type(v.graphic) == "table" then
							...
							if v.drawhat and hatoffsets[v.animationstate] then
								...
								if v.graphic == v.biggraphic or v.animationstate == "grow" then
									...
									elseif v.infunnel then
										offsets = bighatoffsets["jumping"]
									elseif v.underwater and (v.animationstate == "jumping" or v.animationstate == "falling") then
		...
		for i, v in pairs(portals) do
			if v.x1 ~= false then
				...
				if v.x2 == false then
					...
					love.graphics.draw(portalglow, math.floor(((v.x1-1-xscroll)*16+offsetx)*scale), math.floor(((v.y1-yscroll-1)*16+offsety)*scale), rotation, scale, scale, 8, 20)
					love.graphics.setColor(255, 255, 255, 255)
				end

				love.graphics.setColor(unpack(v.portal1color))
				love.graphics.drawq(portalimage, portal1quad[portalframe], math.floor(((v.x1-1-xscroll)*16+offsetx)*scale), math.floor(((v.y1-yscroll-1)*16+offsety)*scale), rotation, scale, scale, 8, 8)
			end
			...
			if v.x2 ~= false then
				...
				if v.x1 == false then
					...
					love.graphics.setColor(255, 255, 255, 80 - math.abs(portalframe-3)*10)
					love.graphics.draw(portalglow, math.floor(((v.x2-1-xscroll)*16+offsetx)*scale), math.floor(((v.y2-yscroll-1)*16+offsety)*scale), rotation, scale, scale, 8, 20)
					love.graphics.setColor(255, 255, 255, 255)
				end

				love.graphics.setColor(unpack(v.portal2color))
				love.graphics.drawq(portalimage, portal2quad[portalframe], math.floor(((v.x2-1-xscroll)*16+offsetx)*scale), math.floor(((v.y2-yscroll-1)*16+offsety)*scale), rotation, scale, scale, 8, 8)
			end
		end
			...
			if customforeground == true then
				...
				if custombackgroundimg[customforeground] then
					for i = 1, #custombackgroundimg[customforeground]  do
						local xscroll = xscroll / ((#custombackgroundimg[customforeground]-i+1) * fscrollfactor + 1)
						if reversescrollfactor() == 1 then
							xscroll = 0
						end
...
function drawplayer(i, x, y, cscale,     offsetX, offsetY, rotation, quadcenterX, quadcenterY, animationstate, underwater, ducking, hats, graphic, quad, pointingangle, shot, upsidedown, colors, lastportal, portal1color, portal2color)
	x = x-6
	...
	if not offsetX then
		...
		v = {offsetX=offsetX, offsetY=offsetY, rotation=rotation, quadcenterX=quadcenterX, quadcenterY=quadcenterY, animationstate=animationstate, underwater=underwater, ducking=ducking, hats=hats, graphic=graphic, quad=quad, pointingangle=pointingangle, shot=shot, upsidedown=upsidedown, colors=colors, lastportal=lastportal, portal1color=portal1color, portal2color=portal2color}
	end
	...
	if type(v.graphic) == "table" then
		...
		if v.drawhat ~= false and hatoffsets[v.animationstate] then
			local offsets = {}
			if v.graphic == v.biggraphic or v.animationstate == "grow" then
				if v.animationstate == "grow" then
					offsets = hatoffsets["grow"]
				...
				if v.underwater and (v.animationstate == "jumping" or v.animationstate == "falling") then
					offsets = hatoffsets["swimming"][v.swimframe]
				elseif v.animationstate == "running" or v.animationstate == "falling"  then
					offsets = hatoffsets["running"][v.runframe]
				elseif v.animationstate == "climbing" then
...
function reachedx(currentx)
	...
	if (SERVER or CLIENT) then
		local um = usermessage:new( "net_reachedx", currentx)
		um:send()
	end
...
function startlevel(level, overwrite)
	...
	for i = 1, #startx do
		if startx[i] == checkpointx[i] and starty[i] == checkpointy[i] then
			table.insert(spawnrestrictions, startx[i])
		end
	end
...
function generatespritebatch()
	...
	for y = 0, ytodraw do
		for x = 1, xtodraw do
			...
			local draw = true
			for i, v in pairs(blockbouncex) do
				if blockbouncex[i] == math.floor(xscroll)+x and blockbouncey[i] == math.min(math.floor(yscroll)+y+1, mapheight) then
					draw = false
				end
			end
			if draw == true then
				local t = lmap[math.floor(xscroll)+x][math.min(math.floor(yscroll)+y+1, mapheight)]
...
function game_keypressed(key, unicode)
	...
	if key == "f6" then
		...
		elseif not editormode and not everyonedead then
			if not love.keyboard.isDown("pause") then
				return
			end
			pausemenuopen = true
			love.audio.pause()
...
function shootportal(plnumber, i, sourcex, sourcey, direction, overwrite, mirrored)
	--check if available
	...
	if (SERVER or CLIENT) then
		if not objects["player"][plnumber].remote then
			local um = usermessage:new( "net_shootportal", plnumber .. "~" .. i .. "~" .. sourcex .. "~" .. sourcey .. "~" .. direction)
			um:send()
	...
	local mirror = false
	if cox and map[cox] and map[cox][coy] and map[cox][coy][1] and tilequads[map[cox][coy][1]].mirror then
		mirror = true
	end
...
function nextlevel(overwrite)
	if not levelfinished then
		return
	end
	if SERVER or CLIENT then
		if SERVER then
			local um = usermessage:new( "client_nextlevel")
			um:send()
		elseif not overwrite then
			return
		end
...
function warpzone(i, overwrite)
	if (SERVER or not CLIENT) or overwrite then
		if SERVER then
			local um = usermessage:new( "client_warpzone", i)
			um:send()
		end
	else
		return
	...
	if not arcade and not displaywarpzonetext and i == 4 then
		marioworld = "M"
	end
...
function savemap(filename)
	...
	for y = 1, mapheight do
		for x = 1, mapwidth do
			...
			if #map[x][y] == 1 then
				if prev == map[x][y][1] and (y ~= mapheight or x ~= mapwidth) then
					mul = mul + 1
				elseif prev == map[x][y][1] and y == mapheight and x == mapwidth then
					mul = mul + 1
					s = s .. prev .. "*" .. mul
				else
					if prev then
...
function traceline(sourcex, sourcey, radians, reportal)
	...
	while currentblock[1]+1 > 0 and currentblock[1]+1 <= mapwidth and (flagx == false or currentblock[1]+1 <= flagx) and (axex == false or currentblock[1]+2 <= axex) and (currentblock[2] > 0 or currentblock[2] >= math.floor(sourcey+0.5)) and currentblock[2] < mapheight+1 do --while in map range
		local oldy = y
		local oldx = x
...
function spawnenemy(x, y)
	...
	for i = 1, #spawnrestrictions do
		if x > spawnrestrictions[i]-6 and x < spawnrestrictions[i]+6 then
			allowenemy = false
		end
	end
	...
	if #t > 1 then
		...
		elseif i == "squid" and allowenemy then
			table.insert(objects["squid"], squid:new(x, y-1/16))

		elseif i == "platform" then
			table.insert(objects["platform"], platform:new(x, y, t)) --Platform

		elseif i == "platformfall" then
			table.insert(objects["platform"], platform:new(x, y, {0, 0, "fall", 0, 0, t[3]})) --Platform fall

		elseif i == "platformbonus" then
			table.insert(objects["platform"], platform:new(x, y, {0, 0, "justright", 2/9+0.001}))

		elseif i == "plant" then
			table.insert(objects["plant"], plant:new(x, y))

		elseif i == "castlefire" then
			table.insert(objects["castlefire"], castlefire:new(x, y, t))

		elseif i == "hammerbro" and allowenemy then
		...
		elseif i == "geldispenser" then
			table.insert(objects["geldispenser"], geldispenser:new(x, y, t))

		elseif i == "upfire" then
			table.insert(objects["upfire"], upfire:new(x, y, t))

		elseif i == "panel" then
			table.insert(objects["panel"], panel:new(x-1, y-1, t))

		elseif i == "textentity" then
			table.insert(textentities, textentity:new(x-1, y-1, t))
...
function game_joystickpressed( joystick, button )
	...
	if arcade then
		...
		elseif button == 7 then
			if arcadeplaying[arcadejoystickmaps[joystick]] then
				arcadeexittimer[arcadejoystickmaps[joystick]] = 0
			end
		end

		arcadetimeouttimers[arcadejoystickmaps[joystick]] = 0
	end
