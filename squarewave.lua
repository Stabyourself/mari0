...
function squarewave:init(x, y, r)
	...
	if #self.r > 0 and self.r[1] ~= "link" then
		self.time1 = squarewavetimefunction(self.r[1])
		table.remove(self.r, 1)
	end
	--TIME2
	if #self.r > 0 and self.r[1] ~= "link" then
		self.time2 = squarewavetimefunction(self.r[1])
		table.remove(self.r, 1)
	end
	--START
	if #self.r > 0 and self.r[1] ~= "link" then
...
function squarewave:draw()
	if self.visible then
		...
		love.graphics.drawq(entitiesimg, entityquads[82].quad, math.floor((self.x-1-xscroll)*16*scale), ((self.y-yscroll-1)*16-8)*scale, 0, scale, scale)
	end
end
