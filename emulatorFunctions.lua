...
function load_level(suspended)
    ...
    -- WHERE IS THE agents OBJECT CREATED -> change agents to agents
    mariolives = {}
    for i = 1, agents do
...
function game_draw()
    for split = 1, #splitscreen do
        ...
        if agents > 1 then
            for i = 1, agents do
                local x = (width*16)/agents/2 + (width*16)/agents*(i-1)
...
function game_update(dt)
    ...
    if autoscroll and minimapdragging == false then
        ...
        for split = 1, #splitscreen do
            ...
            --LEFT
            local i = 1
            while i <= agents and objects["player"][i].dead do
                i = i + 1
            end
