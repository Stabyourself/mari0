...
function menu_draw()
	...
	for y = 1, 15 do
		for x = 1, xtodraw do
			...
			if tilequads[tilenumber].coinblock and tilequads[tilenumber].invisible == false then --coinblock
				love.graphics.drawq(coinblockimg, coinblockquads[spriteset][coinframe], math.floor((x-1)*16*scale), ((y-1)*16-8)*scale, 0, scale, scale)
	...
	love.graphics.drawq(coinanimationimg, coinanimationquads[1][coinframe], uispace*1.5-16*scale, 16*scale, 0, scale, scale)
	...
	for j = 1, players do		
		...
		drawplayer(nil, (startx[pid]*16)+8*(j-1), (starty[pid]*16-12), scale,     v.smalloffsetX, v.smalloffsetY, 0, v.smallquadcenterX, v.smallquadcenterY, "idle", false, false, mariohats[j], v.animations, v.idle[angle], 0, false, false, mariocolors[j], 1, portalcolor[j][1], portalcolor[j][2])
	end
	...
	if gamestate == "menu" then
		...
		for i = start, 9 do
			...
			properprint("online play", 95*scale, 154*scale)
	...
	elseif gamestate == "options" then
		...
		elseif optionstab == 2 then
			...
			drawplayer(nil, 46, 32, scale*2,     v.smalloffsetX, v.smalloffsetY, 0, v.smallquadcenterX, v.smallquadcenterY, "idle", false, false, mariohats[skinningplayer], v.animations, v.idle[angle], 0, false, false, mariocolors[skinningplayer], 1, portalcolor[skinningplayer][1], portalcolor[skinningplayer][2])
			...
			for j = 1, 3 do
				...
				drawplayer(nil, 158, ((j-1)*32)+infmarioY+2, scale,     v.smalloffsetX, v.smalloffsetY, infmarioR, v.smallquadcenterX, v.smallquadcenterY, "jumping", false, false, mariohats[skinningplayer], v.animations, v.jump[angle][1], 0, false, false, mariocolors[skinningplayer], 1, portalcolor[skinningplayer][1], portalcolor[skinningplayer][2])
			end
			...
			if v.colorables then
				...
				properprint(s, 200*scale-string.len(s)*8, 116*scale)
				...
				properprint(s, 200*scale-string.len(s)*8, 126*scale)
				...
				properprint(s, 200*scale-string.len(s)*8, 136*scale)
			end
...
function loadbackground(background)
	...
	if love.filesystem.exists("mappacks/" .. mappack .. "/" .. background) == false then
		...
		if math.mod(entries, mapheight) ~= 0 then
			print("Incorrect number of entries: " .. entries)
			return false
		end
		...
		for i = 1, #t do		
			if string.find(t[i], "*") then --new stuff!
				...
				if #r > 1 and entitylist[tonumber(r[2])] == "spawn" then
					if r[3] == "true" then --all
						startx = {x, x, x, x, x}
						starty = {y, y, y, y, y}
					else
						for i = 1, 5 do
